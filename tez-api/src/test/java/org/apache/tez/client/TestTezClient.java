...
public class TestTezClient {
  ...
  TezClientForTest configureAndCreateTezClient(TezConfiguration conf)
      throws YarnException, IOException, ServiceException {
  ...
  private void _testTezClientSessionLargeDAGPlan(int maxIPCMsgSize, int payloadSize, int amResourceSize,
    ...
    if (shouldSerialize) {
      ...
      Assert.assertTrue(request.hasSerializedRequestPath());
      Assert.assertFalse(request.hasDAGPlan());
      Assert.assertFalse(request.hasAdditionalAmResources());
    } else {
      Assert.assertFalse(request.hasSerializedRequestPath());
      Assert.assertTrue(request.hasDAGPlan());
      Assert.assertTrue(request.hasAdditionalAmResources());
    }
  }
