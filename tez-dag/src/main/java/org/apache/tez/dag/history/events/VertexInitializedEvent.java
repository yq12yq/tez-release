...
public class VertexInitializedEvent implements HistoryEvent {
  ...
  public TimelineEntity convertToTimelineEntity() {
    ...
    initEvt.setTimestamp(initedTime);
    atsEntity.addEvent(initEvt);

    atsEntity.addOtherInfo(ATSConstants.VERTEX_NAME, vertexName);
    atsEntity.addOtherInfo(ATSConstants.INIT_REQUESTED_TIME, initRequestedTime);
    atsEntity.addOtherInfo(ATSConstants.INIT_TIME, initedTime);
    atsEntity.addOtherInfo(ATSConstants.NUM_TASKS, numTasks);
    atsEntity.addOtherInfo(ATSConstants.PROCESSOR_CLASS_NAME, processorName);
