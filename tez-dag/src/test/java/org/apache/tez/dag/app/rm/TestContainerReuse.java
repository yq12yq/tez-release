...
public class TestContainerReuse {
  ...
  public void testReuseConflictLocalResources() throws IOException, InterruptedException, ExecutionException {
    ...
    taskSchedulerEventHandler.handleEvent(new AMSchedulerEventTAEnded(ta211, container2.getId(),
        TaskAttemptState.SUCCEEDED));
    drainableAppCallback.drain();
    verify(taskScheduler).deallocateTask(eq(ta211), eq(true));
    ...
    taskSchedulerEventHandler.handleEvent(lrEvent31);
    TestTaskSchedulerHelpers.waitForDelayedDrainNotify(drainNotifier);
    drainableAppCallback.drain();
    verify(taskSchedulerEventHandler).taskAllocated(eq(ta311), any(Object.class), eq(container2));
    eventHandler.reset();
  ...
  public void testDifferentResourceContainerReuse() throws Exception {
    ...
    taskSchedulerEventHandler.handleEvent(new AMSchedulerEventTAEnded(ta11, container1.getId(),
        TaskAttemptState.SUCCEEDED));
    drainableAppCallback.drain();
    verify(taskScheduler).deallocateTask(ta11, true);
    verify(taskSchedulerEventHandler).taskAllocated(eq(ta12), any(Object.class), eq(container1));
    verify(rmClient, times(0)).releaseAssignedContainer(eq(container1.getId()));
    eventHandler.verifyNoInvocations(AMContainerEventStopRequest.class);
    eventHandler.reset();
    ...
    taskSchedulerEventHandler.handleEvent(new AMSchedulerEventTAEnded(ta13, container2.getId(),
        TaskAttemptState.SUCCEEDED));
    drainableAppCallback.drain();
    verify(taskScheduler).deallocateTask(ta13, true);
    verify(taskSchedulerEventHandler).taskAllocated(eq(ta14), any(Object.class), eq(container2));
    verify(rmClient, times(0)).releaseAssignedContainer(eq(container2.getId()));
    eventHandler.verifyNoInvocations(AMContainerEventStopRequest.class);
    eventHandler.reset();
    ...
    taskSchedulerEventHandler.handleEvent(new AMSchedulerEventTAEnded(ta12, container1.getId(),
        TaskAttemptState.SUCCEEDED));
    drainableAppCallback.drain();
    verify(taskScheduler).deallocateTask(ta12, true);
    verify(rmClient).releaseAssignedContainer(eq(container1.getId()));
    eventHandler.verifyInvocation(AMContainerEventStopRequest.class);
    eventHandler.reset();

    // ta14 finished no pending requests, should release container2.
    taskSchedulerEventHandler.handleEvent(new AMSchedulerEventTAEnded(ta14, container2.getId(),
        TaskAttemptState.SUCCEEDED));
    drainableAppCallback.drain();
    verify(taskScheduler).deallocateTask(ta14, true);
    verify(rmClient).releaseAssignedContainer(eq(container2.getId()));
    eventHandler.verifyInvocation(AMContainerEventStopRequest.class);
    eventHandler.reset();
  ...
  public void testEnvironmentVarsContainerReuse() throws Exception {
    ...
    TezTaskAttemptID taID11 = TezTaskAttemptID.getInstance(TezTaskID.getInstance(vertexID1, 1), 1);
    TaskAttempt ta11 = mock(TaskAttempt.class);
    doReturn("TA ta11").when(ta11).toString();
    AMSchedulerEventTALaunchRequest lrEvent1 = createLaunchRequestEvent(
    ...
    TezTaskAttemptID taID12 = TezTaskAttemptID.getInstance(TezTaskID.getInstance(vertexID1, 2), 1);
    TaskAttempt ta12 = mock(TaskAttempt.class);
    doReturn("TA ta12").when(ta12).toString();
    AMSchedulerEventTALaunchRequest lrEvent2 = createLaunchRequestEvent(
    ...
    TezTaskAttemptID taID13 = TezTaskAttemptID.getInstance(TezTaskID.getInstance(vertexID1, 3), 1);
    TaskAttempt ta13 = mock(TaskAttempt.class);
    doReturn("TA ta13").when(ta13).toString();
    AMSchedulerEventTALaunchRequest lrEvent3 = createLaunchRequestEvent(
    ...
    TezTaskAttemptID taID14 = TezTaskAttemptID.getInstance(TezTaskID.getInstance(vertexID1, 4), 1);
    TaskAttempt ta14 = mock(TaskAttempt.class);
    doReturn("TA ta14").when(ta14).toString();
    AMSchedulerEventTALaunchRequest lrEvent4 = createLaunchRequestEvent(
    ...
    taskSchedulerEventHandler.handleEvent(new AMSchedulerEventTAEnded(ta13, container1.getId(),
        TaskAttemptState.SUCCEEDED));
    TestTaskSchedulerHelpers.waitForDelayedDrainNotify(drainNotifier);
    drainableAppCallback.drain();
    verify(taskScheduler).deallocateTask(ta13, true);
    ...
    TestTaskSchedulerHelpers.waitForDelayedDrainNotify(drainNotifier);
    drainableAppCallback.drain();
    verify(taskSchedulerEventHandler).taskAllocated(eq(ta12), any(Object.class), eq(container2));

    // ta12 finished, cannot reuse container, should release container2
    taskSchedulerEventHandler.handleEvent(new AMSchedulerEventTAEnded(ta12, container2.getId(),
        TaskAttemptState.SUCCEEDED));
    drainableAppCallback.drain();
    verify(taskScheduler).deallocateTask(ta12, true);
    verify(rmClient).releaseAssignedContainer(eq(container2.getId()));
    ...
    TestTaskSchedulerHelpers.waitForDelayedDrainNotify(drainNotifier);
    drainableAppCallback.drain();
    verify(taskSchedulerEventHandler).taskAllocated(eq(ta14), any(Object.class), eq(container3));

    // ta14 finished, should release container3
    taskSchedulerEventHandler.handleEvent(new AMSchedulerEventTAEnded(ta14, container3.getId(),
        TaskAttemptState.SUCCEEDED));
    drainableAppCallback.drain();
    verify(taskScheduler).deallocateTask(ta14, true);
