...
public class TestATSService {
  ...
  public void setup() throws Exception {
    ...
    atsService = new ATSService(appContext);
    conf = new Configuration(false);
    conf.setLong(TezConfiguration.YARN_ATS_EVENT_FLUSH_TIMEOUT_MILLIS,
        1000l);
    atsInvokeCounter = 0;
    atsService.init(conf);
    atsService.timelineClient = mock(TimelineClient.class);
    atsService.start();
    when(appContext.getClock()).thenReturn(clock);
    when(atsService.timelineClient.putEntities(any(TimelineEntity.class))).thenAnswer(
        new Answer<Object>() {
          @Override
  ...
  @Test(timeout=20000)
  public void testATSServiceShutdown() {
    ...
    for (int i = 0; i < 20; ++i) {
      atsService.handle(historyEvent);
    }
